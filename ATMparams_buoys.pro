...
;
; CALL SEQUENCE: 
;    GET_ALL_SCENE_PARAMS,   folderpath, $                    ; path where read-in files and result.tifs are (include '\' after folder)
...
;                            GETDIST2CLOUD=getdist2cloud, $   ; if set, the cloud mask will be used to calculate distances from the buoy to the cloud pixels
;                            
;                                        
; RESTRICTIONS:
...
;
PRO ATMparams_buoys, folderpath, results_filename, MYLATLONS=mylatlons, GETDIST2CLOUD=getdist2cloud
  ...
  FOR i=0,N_ELEMENTS(buoylat)-1 DO BEGIN
    ...
    IF KEYWORD_SET(mylatlons) EQ 0 THEN BEGIN
      IF usebuoy[0,i] EQ 0 THEN BEGIN
        ...
        CONTINUE
      ENDIF
    ENDIF ELSE IF KEYWORD_SET(getdist2cloud) THEN BEGIN
    ...
    ENDIF  ELSE BEGIN
        PRINT, 'Obtaining parameters for ',filename[i],'.....'
        params[*,i]= GET_ATM_PARAMS(folderpath,filename[i],buoylat[i],buoylon[i], whichLandsat[i])   
        ENDELSE
